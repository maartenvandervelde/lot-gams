...
The general trend over time seems to be that pupil dilation increases (captured by the fixed `Time` smooth, although it is not significant in this particular model).
...
It is clear from the fitted effects plot above that there are sizeable baseline differences in pupil size *within* participants, which is to be expected as pupil size can fluctuate naturally over a task. A random `Subject` effect cannot capture this as it is the same for all the subject's trials. Furthermore, in many cases the difference in baseline pupil size is already present at $t = 0$, meaning that it has little to do with the item itself, and that a random `Item` effect is therefore ineffective at capturing this variance. The `Event` intercept can account for these general fluctuations in pupil size that are not consistently tied to a subject or item much better since the intercept is fitted to each trial individually.
...
It could be further improved with a random factor smooth for each `Event` over `Time` (instead of the random intercept for `Event`). This would allow it to fit differences in the magnitude of the pupil dilation better (with a random intercept the model still assumes that the change in pupil size is the same regardless of how big the pupil is at the start of the trial, which does not seem like a reasonable assumption).
...
This column marks the first observation in each `Event` (trial).
...
The smooth terms also look similar to before:
```{r}
par(mfrow=c(2,2), cex=0.75)
...
The distribution of the residuals is still wider than a normal distribution, but the autocorrelation in the residuals has been reduced:
```{r}
par(mfrow = c(1,2))
qqnorm(resid(m4b))
qqline(resid(m4b))
...
Based on its lower AIC score, the model that includes the AR1 model is preferred.
...
Perhaps including a higher-order AR($\rho$) model would reduce the unexplained autocorrelation in the most extreme cases.
...
Based on the summary alone it is hard to conclude much about the differences between conditions; none of the intercept differences between conditions is significant, and we know that the `Time` smooth is significantly different from a horizontal line at $y = 0$ in all four conditions, but not how it differs *between* conditions.
We can plot the fitted effects for all conditions, but as expected they look very similar:
...
The `plot_diff()` function can show pairwise comparisons between conditions, and will highlight in red where there are significant differences:
```{r}
...
Another way to test for differences between conditions would be to use contrast coding in the model, where one condition is set to be the reference level and significant effects for other conditions indicate that they are different from the reference condition.
...
The intercept represents the condition in which each of the binary predictors is zero: *other-oriented:A2*.
...
They describe the `Time` smooth belonging to each of the four conditions. From top to bottom: *other-oriented:A2*, *other-oriented:A1*, *self-oriented:A2*, *self-oriented:A1*.
...
- the actor is introduced first rather than second (when an other-oriented action is shown);
- a self-oriented action is shown rather than an other-oriented action (when the actor is introduced second);
- a self-oriented action is shown rather than an other-oriented action, and the actor is introduced first rather than second.
...
- Pupil dilation is greater from about 1000 ms to 1500 ms if the actor is introduced first rather than second (when an other-oriented action is shown).
- Pupil dilation appears to be the same when the actor is introduced second, regardless of whether the action shown is self-oriented or other-oriented. The fact that this contrast is significant in the model suggests that the dilation difference at the very end of the trial (after 3000 ms) is significant (the bottom-right plot in the condition comparison for model 5 corroborates this), but I would consider this a spurious effect.
- The pupil dilates less between about 500 ms and 1500 ms when a self-oriented action is accompanied by a sentence in which the actor is introduced first, compared to an other-oriented action accompanied by a sentence in which the actor is introduced second.


*Use model comparisons (functions compareML and AIC) to test whether the binary interaction predictor IsSelfA1 is significantly contributing to the model. Provide the code for the model comparison in your assignment. (Note that actually model m6 and the new model should be run with method="ML" included for fixed effects comparisons, but for time reasons we will ignore this for the current assignment.)*
...
The full model uses more degrees of freedom, but has a significantly lower AIC score than the model that leaves out the `IsSelfA1` predictor.
The plots also showed that this predictor is important to include in the model: while the two components `IsA1` and `IsSelf` by themselves predict an increase or no change in pupil dilation relative to the reference condition, the interaction of these components actually makes the opposite prediction: pupil dilation  *decreases* relative to the reference condition when both are true. It is therefore necessary to include the interaction in the model.

<div class="panel panel-info">
  <div class="panel-heading">*What is your conclusion based on the model comparisons, the plots, and the summary statistics?*</div>
  <div class="panel-body">The full model uses more degrees of freedom, but has a significantly lower AIC score than the model that leaves out the `IsSelfA1` predictor.
The plots also showed that this predictor is important to include in the model: while the two components `IsA1` and `IsSelf` by themselves predict an increase or no change in pupil dilation relative to the reference condition, the interaction of these components actually makes the opposite prediction: pupil dilation  *decreases* relative to the reference condition when both are true. It is therefore necessary to include the interaction in the model.
</div>
</div>
